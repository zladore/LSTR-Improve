...
class TextCalibrationBank:
    ...
    def _align_frame_feats(self, session: str, vis_tensor: torch.Tensor, query_indices=None) -> torch.Tensor:
        frame_feats_full = self._load_frame_features(session)

        if query_indices is not None:
            idx = self._to_index_tensor(query_indices)

            if idx.numel() != vis_tensor.shape[0]:
                raise ValueError(
                    f"query_indices/visual length mismatch for {session}: "
                    f"indices={idx.numel()} vs vis={vis_tensor.shape[0]}"
                )
            ...
            if frame_feats.shape[0] != vis_tensor.shape[0]:
                if self.strict_length:
                    raise ValueError(
                        f"text/visual length mismatch for {session}: "
                        f"frame_text={frame_feats.shape[0]} vs vis={vis_tensor.shape[0]}"
                    )
                else:
                    t = min(frame_feats.shape[0], vis_tensor.shape[0])
