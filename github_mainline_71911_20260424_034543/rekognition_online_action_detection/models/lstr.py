...
class LSTR(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.work_enabled:
            self.with_text = getattr(self.feature_head_work, 'with_text', False)
            self.d_model = self.feature_head_work.d_model
        else:
            self.with_text = getattr(self.feature_head_long, 'with_text', False)
            self.d_model = self.feature_head_long.d_model
    ...
    def _parse_forward_extra(self, extra):
        text_inputs = None
        ...
        if self.with_text:
            if len(extra) == 1:
                ...
                text_inputs, memory_key_padding_mask = extra
            else:
                raise RuntimeError(f'Invalid input count for threestream forward: got {len(extra)} extra args')
        else:
            if len(extra) == 1:
                memory_key_padding_mask = extra[0]
            elif len(extra) > 1:
                raise RuntimeError(f'Invalid input count for non-text forward: got {len(extra)} extra args')

        return text_inputs, memory_key_padding_mask

    def forward(self, visual_inputs, motion_inputs, *extra):
        text_inputs, memory_key_padding_mask = self._parse_forward_extra(extra)

        if self.long_enabled:
            # Compute long memories
            long_visual_inputs = visual_inputs[:, :self.long_memory_num_samples]
            long_motion_inputs = motion_inputs[:, :self.long_memory_num_samples]
            long_text_inputs = text_inputs[:, :self.long_memory_num_samples] if self.with_text else None
        ...
        if self.work_enabled:
            ...
            work_visual_inputs = visual_inputs[:, self.long_memory_num_samples:]
            work_motion_inputs = motion_inputs[:, self.long_memory_num_samples:]
            work_text_inputs = text_inputs[:, self.long_memory_num_samples:] if self.with_text else None
            ...
            # Build mask
            mask = tr.generate_square_subsequent_mask(work_memories.shape[0])
...
class LSTRStream(LSTR):
    ...
    def _parse_stream_extra(self, extra):
        long_text_inputs = None
        work_text_inputs = None
        memory_key_padding_mask = None

        if self.with_text:
            if len(extra) != 3:
                raise RuntimeError(
                    f'Invalid input count for threestream stream_inference: expected 3 extra args, got {len(extra)}'
                )
            long_text_inputs, work_text_inputs, memory_key_padding_mask = extra
        else:
            if len(extra) != 1:
                raise RuntimeError(
                    f'Invalid input count for non-text stream_inference: expected 1 extra arg, got {len(extra)}'
                )
            memory_key_padding_mask = extra[0]

        return long_text_inputs, work_text_inputs, memory_key_padding_mask
    ...
    def stream_inference(self,
                         ...
                         work_motion_inputs,
                         *extra):
        long_text_inputs, work_text_inputs, memory_key_padding_mask = self._parse_stream_extra(extra)
        ...
        if (long_visual_inputs is not None) and (long_motion_inputs is not None):
            ...
            # Encode long memories
            long_memories = self.enc_modules[0].stream_inference(
                enc_queries[0], long_memories, pos,
                memory_key_padding_mask=memory_key_padding_mask
            )
