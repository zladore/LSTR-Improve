...
def assert_and_infer_cfg(cfg, args):
    ...
    # Input assertions
    assert cfg.INPUT.MODALITY in ['visual', 'motion', 'twostream', 'threestream']

    if cfg.INPUT.MODALITY == 'threestream':
        assert cfg.INPUT.TEXT_FEATURE != '', 'INPUT.TEXT_FEATURE must be set for threestream'
        assert cfg.INPUT.TEXT_FEATURE_DIM > 0, 'INPUT.TEXT_FEATURE_DIM must be > 0 for threestream'
