...
def do_lstr_stream_inference(cfg, model, device, logger):
    ...
    with_text = cfg.INPUT.MODALITY == 'threestream'
    if with_text:
        if osp.isabs(cfg.INPUT.TEXT_FEATURE):
            text_feature_root = cfg.INPUT.TEXT_FEATURE
        else:
            text_feature_root = osp.join(cfg.DATA.DATA_ROOT, cfg.INPUT.TEXT_FEATURE)
    else:
        text_feature_root = None
    ...
    with torch.no_grad():
        for session_idx, session in enumerate(cfg.DATA.TEST_SESSION_SET):
            visual_inputs = np.load(
                osp.join(cfg.DATA.DATA_ROOT, cfg.INPUT.VISUAL_FEATURE, session + '.npy'),
                mmap_mode='r'
            )
            motion_inputs = np.load(
                osp.join(cfg.DATA.DATA_ROOT, cfg.INPUT.MOTION_FEATURE, session + '.npy'),
                mmap_mode='r'
            )
            if with_text:
                text_inputs = np.load(
                    osp.join(text_feature_root, session + '.npy'),
                    mmap_mode='r'
                )
            else:
                text_inputs = None
            ...
            for work_start, work_end in zip(range(0, target.shape[0] + 1),
                ...
                # Get target
                target_slice = target[::work_memory_sample_rate]
                ...
                if with_text:
                    work_text_inputs = to_device(text_inputs[work_indices])
                else:
                ...
                if long_end == -1:
                    ...
                    if with_text:
                        long_text_inputs = to_device(text_inputs[long_indices])
                    else:
                    ...
                    if with_text:
                        long_text_inputs = to_device(text_inputs[[long_end]])
                    else:
                        long_text_inputs = None
                else:
                ...
                if work_start == 0:
                    gt_targets.extend(list(target_slice[:work_end]))
                    pred_scores.extend(list(score))
                else:
                    gt_targets.append(list(target_slice[work_end - 1]))
