...
class LSTRDataLayer(data.Dataset):
    ...
    def __init__(self, cfg, phase='train'):
        ...
        self.with_text = cfg.INPUT.MODALITY == 'threestream'
        self.text_feature = cfg.INPUT.TEXT_FEATURE
        if self.with_text:
            if osp.isabs(self.text_feature):
                self.text_feature_root = self.text_feature
            else:
                self.text_feature_root = osp.join(self.data_root, self.text_feature)
        else:
            self.text_feature_root = None
    ...
    def __getitem__(self, index):
        ...
        if self.with_text:
            text_inputs = np.load(
                osp.join(self.text_feature_root, session + '.npy'), mmap_mode='r')
        else:
            text_inputs = None
        ...
        if self.with_text:
            work_text_inputs = text_inputs[work_indices]
        else:
        ...
        if self.long_memory_num_samples > 0:
            ...
            if self.with_text:
                long_text_inputs = text_inputs[long_indices]
            else:
        ...
        if long_visual_inputs is not None and long_motion_inputs is not None:
            ...
            if self.with_text:
                fusion_text_inputs = np.concatenate((long_text_inputs, work_text_inputs))
            else:
                fusion_text_inputs = None
        else:
            fusion_visual_inputs = work_visual_inputs
            fusion_motion_inputs = work_motion_inputs
            fusion_text_inputs = work_text_inputs if self.with_text else None
        ...
        if self.with_text:
            fusion_text_inputs = torch.as_tensor(fusion_text_inputs.astype(np.float32))

        if memory_key_padding_mask is not None:
            memory_key_padding_mask = torch.as_tensor(memory_key_padding_mask.astype(np.float32))
            if self.with_text:
                return fusion_visual_inputs, fusion_motion_inputs, fusion_text_inputs, memory_key_padding_mask, target
            else:
                return fusion_visual_inputs, fusion_motion_inputs, memory_key_padding_mask, target
        else:
            if self.with_text:
                return fusion_visual_inputs, fusion_motion_inputs, fusion_text_inputs, target
            else:
...
class LSTRBatchInferenceDataLayer(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if memory_key_padding_mask is not None:
            ...
            if self.with_text:
                return (
                    fusion_visual_inputs, fusion_motion_inputs, fusion_text_inputs,
                    memory_key_padding_mask, target, session, work_indices, num_frames
                )
            else:
                return (
                    fusion_visual_inputs, fusion_motion_inputs,
                    memory_key_padding_mask, target, session, work_indices, num_frames
                )
        else:
            if self.with_text:
                return (
                    fusion_visual_inputs, fusion_motion_inputs, fusion_text_inputs,
                    target, session, work_indices, num_frames
                )
            else:
                return (
                    fusion_visual_inputs, fusion_motion_inputs,
